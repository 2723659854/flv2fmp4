...
class MP4 {
    ...
    public static function moov($meta) {
        ...
        foreach ($meta as $m) {
            if (isset($m['codecWidth'])) {
                $videoMeta = $m;
            } else if (isset($m['audioSampleRate'])) {
                $audioMeta = $m;
            }
        }
    ...
    public static function avc1($meta) {
        ...
        $data = [
            // VisualSampleEntry header according to ISO BMFF spec
            0x00, 0x01,  // data_reference_index (2 bytes)
            ...
            ($height >> 8) & 0xFF,
            $height & 0xFF,
            0x00, 0x48, 0x00, 0x00,  // horizontal resolution (dpi)
            0x00, 0x48, 0x00, 0x00,  // vertical resolution (dpi)
            0x00, 0x00, 0x00, 0x00,  // reserved
            0x00, 0x01,              // frame_count
            0x00, 0x00,              // compressorname length
            0x00, 0x18,              // depth
