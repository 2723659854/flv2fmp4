...
$segmentFile = __DIR__.'/output/segment_2.m4s';
$data = file_get_contents($segmentFile);
$bytes = unpack('C*', $data);
$array = array_values($bytes);

echo "Segment 2 file size: " . count($array) . " bytes\n";
...
for ($i = 0; $i < count($array) - 4; $i++) {
    if ($array[$i] == 0x6D && $array[$i+1] == 0x64 && $array[$i+2] == 0x61 && $array[$i+3] == 0x74) {
        ...
        echo "Last 50 bytes of mdat data: " . implode(', ', array_slice($array, $i + 8 + $size - 50, 50)) . "\n";
        break;
    }
}
?>